...
function OpenPanel(elem) {
    ...
    if(document.getElementById(elem.id).children.item(0).innerHTML.length==2){
    localStorage.setItem("rankoperationalSelected", document.getElementById(elem.id).children.item(0).innerHTML.slice(0, 1));
    } else {
        localStorage.setItem("rankoperationalSelected", document.getElementById(elem.id).children.item(0).innerHTML.slice(0, 2));
    }

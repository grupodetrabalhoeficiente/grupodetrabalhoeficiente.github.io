...
function openWitness() {
    window.location.href = "../html/RegistarTestemunhas.html";
}
...
function openPedidos() {
    if (localStorage.getItem('help') == undefined) {
        window.location.href = "../html/PedidoDeAjuda1.html";
    }
}

function openPresence() {
    if (localStorage.getItem('presence') == undefined) {
        window.location.href = "../html/OperationalsPresence.html";
    }
}

function openMateriais() {
    if (localStorage.getItem('materials') == undefined) {
        window.location.href = "../html/ConfirmarMateriais.html"
    }
}
    ...
    fetch('https://bdc5dcf6bca04b39ab10a706cdb79f29.vfs.cloud9.us-east-1.amazonaws.com/occurrences/' + id_occurrence + '/evaluations')
        ...
        .then((out) => {
            ...
            $.each(out, function(index, value) {
                ...
                txt += '<div class="accordion-body"> <i class="fas fa-user"></i><strong> ID:</strong> #' + value.id_operational + '<br> <strong><i class="fas fa-fire-extinguisher"></i> Especialidade:</strong> ' + value.speciality + '<br>';
                txt += '<i class="fas fa-phone"></i><strong> Telemovel:</strong><a href="tel:'+value.phone_number+'"> ' + value.phone_number + '</a><br>';
                txt += ' </div>  </div>  </div> </div>'
                count++;
            });
        ...
        .then((out) => {
            $.each(out, function(index, value) {
                ...
                if(value.arrival!=null){
                    document.getElementById("hourId").value= value.arrival.slice(11, 13);
                document.getElementById("minuteId").value = value.arrival.slice(14, 16);
                }
            })
        }).catch(err => console.error(err));
            ...
            if (Number(localStorage.getItem('stage')) === 3) {
                ...
                if (Number(localStorage.getItem('stage')) === 4) {
                    ...
                    if (Number(localStorage.getItem('stage')) === 5) {
                        window.location.replace('../html/UtilizacaoMateriais.html');
                    }
                    else {
                        if (Number(localStorage.getItem('stage')) === 6) {
                            window.location.replace('../html/AvaliarOperacionais.html');
                        }
                    }
                }
            }
